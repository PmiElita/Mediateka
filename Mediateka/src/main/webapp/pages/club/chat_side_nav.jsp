...
<nav>
        ...
        <div class="card-panel grey lighten-5 z-depth-1" style="padding:5px;">
          <div class="row valign-wrapper" style="line-height: 20px; margin-bottom: 5px;">
            <div class="col s2">
              ...
              <img src="media/book ava/images/dWxrVkt5dbECsuBp.jpg" alt="" class="circle responsive-img"> <!-- notice the "circle" class -->
           
            </div>
            
            
            <div class="col s10">
              <span class="user_name"> <c:out value="${chatMessage.value }"></c:out> : </span>
              <span class="black-text">
             <c:out value="${chatMessage.key.text}"></c:out> 
              </span>
            </div>
            </div>
           ...
           <div class="panel input-area ">
            <textarea id="messageInput" class="materialize-textarea"  placeholder="Message"
                    ></textarea>
         
            <button class=" col s4 offset-s9 btn-flat" onclick="sendMessage();" >Send</button>
  ...
  <a href="#" class="chat-side-button"  data-activates="sideNav"  id="but"><i id="collapse-icon" class="mdi-communication-message"></i></a>
<script>
$( document ).ready(function() {
	  document.getElementById("messageArea").style.height=($(window).height()-293)+'px';
	$('#but').sideNav({
...
window.setInterval(function(){
	  if (lastMessageTime!=null){
		  if (new Date().getTime()-lastMessageTime.getTime()>277000){
			  var jsonObj = {"userId" : userId,"clubId" : clubId, "message" : ""};
...
function connect () {
    ...
    chatClient.onmessage = function (event) {
        ...
        $.ajax({
			...
			type: 'get',
			dataType: 'json',
			data: {"userId" : jsonObj.userId},
		    complete : function(data) {
		    	if (data.responseJSON.userName!=null){
        		showMessage(data.responseJSON.userName,jsonObj.message);
		    	}
        	}
	 ...
	 if (userName!=null){
         ...
         "<div class=\"card-panel grey lighten-5 z-depth-1\" style=\"padding:5px;\">"+
           "<div class=\"row valign-wrapper\" style=\"line-height: 20px; margin-bottom: 5px;\">"+
             ...
             "</div>"+
             "<div class=\"col s10\">"+
               "<span class=\"user_name\">"+ escapeHtml(userName)+ " : </span>"+
               "<span class=\"black-text\">"+
...
function sendMessage() {
   ...
   var clubId = document.getElementById("clubId").innerHTML;
    ...
    if (message !== "") {
        var jsonObj = {"userId" : userId,"clubId" : clubId, "message" : message};
        chatClient.send(JSON.stringify(jsonObj));
        inputElement.value = "";
    }
    inputElement.focus();
	 ...
	 var chatHeight = chat.scrollHeight;
  	...
  	if (chat.scrollTop ===0) {
  		...
  		$.ajax({
  			...
  			type: 'get',
  			dataType:'json',
  			data: {"clubId": clubId, "index":index},
  		    complete: function (data){
  		    	for (var i=0; i<data.responseJSON.length; i++){
  		          ...
  		          "<div class=\"card-panel grey lighten-5 z-depth-1\" style=\"padding:5px;\">"+
  		           "<div class=\"row valign-wrapper\" style=\"line-height: 20px; margin-bottom: 5px;\">"+
  		             ...
  		             "</div>"+
  		             "<div class=\"col s10\">"+
  		               "<span class=\"user_name\">"+ escapeHtml(data.responseJSON[i].userName)+ " : </span>"+
  		               "<span class=\"black-text\">"+
  		             escapeHtml(data.responseJSON[i].message)+
  		               "</span></div> </div>  </div> </div>" + chat.innerHTML;
  		    	}
  		    	index=index+1;
  		    	
  		    	chat.scrollTop = chat.scrollHeight- chatHeight;
